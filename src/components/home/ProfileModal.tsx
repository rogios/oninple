...
export default function ProfileModal({
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-[#1F2937] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto overflow-x-hidden shadow-2xl [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      ...
      >
        ...
        <div className="relative">
          ...
          <div
            className="absolute -bottom-12 left-1/2 -translate-x-1/2 w-24 h-24 rounded-full border-4 border-white dark:border-[#1F2937] overflow-hidden flex items-center justify-center text-white font-black text-2xl shadow-lg shrink-0"
        ...
        <div className="px-6 pb-6 pt-14 min-w-0 overflow-x-hidden">
          ...
          <div className="text-center mb-5">
            <div className="flex items-center justify-center gap-1.5 mb-2">
              <h2 className="text-xl font-black text-[#111111] dark:text-[#F9FAFB]">{channel.channel_name}</h2>
              {channel.is_verified && (
            ...
            <div className="flex items-center justify-center gap-1.5 flex-wrap min-w-0 w-full">
              ...
              {channel.can_collaborate ? (
                ...
                <span className="text-xs font-semibold text-gray-400 dark:text-[#6B7280] bg-gray-50 dark:bg-[#374151] border border-gray-200 dark:border-[#4B5563] px-2.5 py-0.5 rounded-full">협업마감</span>
              )}
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-5">
              {[
                ...
                <div key={label} className="bg-gray-50 dark:bg-[#374151] rounded-xl p-3 text-center">
                  <div className={`text-base font-black leading-tight ${red ? "text-[#E8292E]" : "text-[#111111] dark:text-[#F9FAFB]"}`}>
                    {value}
                  </div>
                  <div className="text-[10px] text-gray-400 dark:text-[#6B7280] mt-0.5">{label}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {(vid1 || vid2) && (
            <div className="mb-5">
              <p className="text-xs font-bold text-gray-400 dark:text-[#6B7280] uppercase tracking-wide mb-2">대표 영상</p>
          ...
          {isEditor && (
            <div className="space-y-4 mb-5">
              {channel.work_fields && channel.work_fields.length > 0 && (
                <div>
                  <p className="text-xs font-bold text-gray-400 dark:text-[#6B7280] uppercase tracking-wide mb-2">작업 분야</p>
              ...
              {channel.ai_tools && channel.ai_tools.length > 0 && (
                <div>
                  <p className="text-xs font-bold text-gray-400 dark:text-[#6B7280] uppercase tracking-wide mb-2">보유 AI 툴</p>
                  <div className="flex gap-1.5 flex-wrap">
                    {channel.ai_tools.map((t) => (
                      <span key={t} className="text-xs text-gray-700 dark:text-[#9CA3AF] bg-gray-100 dark:bg-[#374151] px-2.5 py-1 rounded-full">{t}</span>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div className="space-y-2.5 mb-5 text-sm">
              {channel.content_format && (
                <div className="flex gap-3">
                  <span className="text-gray-400 dark:text-[#6B7280] w-20 shrink-0">콘텐츠 형식</span>
                  <span className="text-gray-700 dark:text-[#F9FAFB]">{channel.content_format}</span>
                </div>
              )}
              {channel.audience_gender && (
                <div className="flex gap-3">
                  <span className="text-gray-400 dark:text-[#6B7280] w-20 shrink-0">주요 시청자</span>
                  <span className="text-gray-700 dark:text-[#F9FAFB]">
                    {channel.audience_gender}
          ...
          <div className="flex flex-col gap-2.5">
              ...
              <a
                ...
                className={`flex items-center justify-center gap-2 w-full py-3 rounded-xl border-2 font-semibold text-sm transition-colors ${
                  channel.platform === "instagram"
                    ? "border-pink-500 text-pink-500 hover:bg-pink-50 dark:hover:bg-pink-900/20"
                    : channel.platform === "tiktok"
                    ? "border-[#010101] dark:border-gray-400 text-[#010101] dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#374151]"
                    : "border-[#FF0000] text-[#FF0000] hover:bg-red-50 dark:hover:bg-red-900/20"
                }`}
              >
            ...
            {isEditor && channel.portfolio_url && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full py-3 rounded-xl border-2 border-[#6C5CE7] text-[#6C5CE7] font-semibold text-sm hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors"
              >
                <ExternalLink size={15} />
                포트폴리오 보기
              </a>
            ...
            {channel.kakao_open_chat && (
              channel.is_verified ? (
                ...
                <button
                  type="button"
                  disabled
                  className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-gray-100 dark:bg-[#374151] text-gray-400 dark:text-[#6B7280] font-bold text-sm cursor-not-allowed"
                >
                  <MessageCircle size={15} />
                  본인 채널 소유 확인 후 이용 가능
                </button>
          ...
          <div className="flex justify-end mt-4">
            <button type="button" className="flex items-center gap-1 text-xs text-gray-300 dark:text-[#374151] hover:text-gray-500 dark:hover:text-[#6B7280] transition-colors">
              <Flag size={11} />
              신고하기
            </button>
          </div>
