...
type Platform = "youtube" | "instagram" | "tiktok" | "editor";
...
export default function ChannelNewForm({
  ...
  const [feedThumb2, setFeedThumb2] = useState<FeedThumb | null>(null);
  ...
  function validate(): boolean {
    ...
    if (form.bio.length > (isEditor ? 300 : 500)) {
      errors.bio = `${isEditor ? 300 : 500}자 이내로 입력해주세요.`;
    }

    setFieldErrors(errors);

    if (Object.keys(errors).length > 0) {
      console.log("[ChannelNewForm] 유효성 검사 실패:", errors);
      // 첫 번째 에러 필드로 스크롤
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    let profileImageUrl = fetchedProfileUrl ?? "";
    ...
    if (result.error) {
      console.error("[ChannelNewForm] 저장 실패:", result.error);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <button
              type="button"
              onClick={() => fileRef.current?.click()}
              className="text-sm font-medium text-[#E8292E] hover:underline"
            >
              이미지 업로드
            </button>
        ...
        <>
          ...
          <Field label="카카오 오픈채팅 링크">
            <Input
              ...
              value={form.kakao_open_chat}
              onChange={(e) => set("kakao_open_chat", e.target.value)}
            />
          </Field>
        </>
      ) : (
          ...
          <div className="bg-gray-50 rounded-2xl p-5 space-y-4">
            ...
            <Field label="성별 비율">
              <RadioGroup
                ...
                onChange={(v) => set("audience_gender", v)}
              />
            </Field>
          </div>
        </>
          ...
          {form.content_keywords.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {form.content_keywords.map((kw) => (
                <span
                  key={kw}
                  className="flex items-center gap-1.5 bg-[#111111] text-white text-xs font-medium px-3 py-1.5 rounded-full"
                >
                  {kw}
      ...
      <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
        ...
        <button
          type="button"
          onClick={() => set("can_collaborate", !form.can_collaborate)}
          className="text-[#E8292E] transition-colors"
        >
          {form.can_collaborate ? (
            <ToggleRight size={36} />
          ) : (
