              ...
              <ul className="space-y-3">
                {card.benefits.map((b) => (
                  <li key={b} className="flex items-start gap-2.5">
                    ...
                    <span className="text-sm text-gray-700 dark:text-[#9CA3AF] leading-snug">{b}</span>
                  </li>
                ))}
              </ul>
            </div>
...
function FeaturesSection() {
  return (
    <section className="py-20 sm:py-28 bg-white dark:bg-[#1F2937]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {FEATURES.map((f) => (
            <div key={f.title} className="bg-white dark:bg-[#111827] rounded-2xl p-7 border border-gray-100 dark:border-[#374151] shadow-sm">
              ...
              <p className="text-sm text-gray-700 dark:text-[#9CA3AF] leading-relaxed">{f.desc}</p>
            </div>
          ))}
        </div>
      </div>
